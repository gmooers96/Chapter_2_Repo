...
from train_fully_conv_SHIELD import encoder_gen, decoder_gen, kl_reconstruction_loss, ELBO_Test, ELBO_Test_Big, PRIOR_Test_Big, PRIOR_Test_Norm
import numpy as np
import gc 
...
def reconstruct_targets_paper(vae, test_data, targets, id, dataset_max, dataset_min):
    ...
    for target in targets:
        ...
        sample_mean = sample_mean_var[0, :170 * 79]
        sample_log_var = sample_mean_var[0, 170 * 79:]
        ...
        max_sample = np.max(sample.reshape((1170 * 79,)))
        ...
        min_sample = np.min(sample.reshape((170 * 79,)))
        print("min of original", min_sample)

        # Reshape reconstructed sample 
        recon_mean = recon_mean.reshape((79, 170))
        recon_var = recon_var.reshape((79, 170))
...
def reconstruct_targets(vae, test_data, targets, id, dataset_max, dataset_min):
    ...
    for target in targets:
        ...
        max_recon_var = np.max(recon_var)
        print("max of reconstructed", max_reconstructed)
        max_sample = np.max(sample.reshape((170 * 79,)))
        print("max of original", max_sample)
        min_reconstructed = np.min(recon_mean)
        min_recon_var = np.min(recon_var)
        print("min of reconstructed", min_reconstructed)
...
def sample_reconstructions(vae, train_data, test_data, id, dataset_max, dataset_min): 
    ...
    for i in range(5):
        ...
        print("original sample", sample.reshape((170 * 79,)))
        print("reconstructed sample", recon_sample)
        print(np.max(np.abs(sample.reshape((170 * 79,)) - recon_sample)))
        ...
        min_sample = np.min(sample.reshape((170 * 79,)))
        print("min of original", min_sample)
        recon_sample = recon_sample.reshape((79, 170))
...
def sample_latent_space_var(vae_encoder, train_data, test_data, id, dataset_min, dataset_max, dataset_type): 
    ...
    #np.save("/fast/gmooers/gmooers_git/CBRAIN-CAM/MAPS/DYAMOND_VAEs/model_graphs/latent_space/SHIELD/Small_GEM_Interp_For_SHIELD_Full_1024D_Latent_Space__{}".format(id), z_test)
    np.save("model_graphs/latent_space/Scree/Z_Test_SHIELD_Full_1024D_Latent_Space__{}".format(id), z_test)
    np.save("model_graphs/latent_space/Scree/Mean_Test_SHIELD_Full_1024D_Latent_Space__{}".format(id), test_mean)
    np.save("model_graphs/latent_space/Scree/Log_Var_Test_SHIELD_Full_1024D_Latent_Space__{}".format(id), test_log_var)
    
    np.save("model_graphs/latent_space/Scree/Z_Train_SHIELD_Full_1024D_Latent_Space__{}".format(id), z_train)
    np.save("model_graphs/latent_space/Scree/Mean_Train_SHIELD_Full_1024D_Latent_Space__{}".format(id), train_mean)
    np.save("model_graphs/latent_space/Scree/Log_Var_Train_SHIELD_Full_1024D_Latent_Space__{}".format(id), train_log_var)
    # Apply scaling and tsne 
    sc = StandardScaler()
    ...
    #np.save("/fast/gmooers/gmooers_git/CBRAIN-CAM/MAPS/DYAMOND_VAEs/model_graphs/latent_space/SHIELD/Small_GEM_Interp_For_SHIELD_3D_PCA_Latent_Space__{}".format(id), z_test_pca)
    np.save("model_graphs/latent_space/Scree/SHIELD_3D_PCA_Latent_Space__{}".format(id), z_test_pca)
...
def sample_frob_norm(vae, decoder, vae_encoder, train_data, test_data, id): 
    ...
    sample_mean = sample_mean_var[:, :170 * 79]
    truths = np.reshape(test_data, (len(test_data),170 * 79))
...
def sample_anon_detect(vae, decoder, vae_encoder, train_data, test_data, id, dataset_min, dataset_max, dataset_type): 
    ...
    sample_mean_var = decoder.predict(z_test) 
    sample_mean = sample_mean_var[:, :170 * 79]
    sample_log_var = sample_mean_var[:, 170 * 79:]
    ...
    np.save("/fast/gmooers/gmooers_git/CBRAIN-CAM/MAPS/DYAMOND_VAEs/model_graphs/Anomalies/Matrix/SHIELD/Big_SHIELD_ELBO_{}.npy".format(id),something)
    print("completed")
...
def sample_prior_detect(vae, decoder, vae_encoder, train_data, test_data, id, dataset_min, dataset_max, dataset_type): 
    ...
    #gc.collect()
    #print(elbo.shape)
    np.save("/fast/gmooers/gmooers_git/CBRAIN-CAM/MAPS/DYAMOND_VAEs/model_graphs/Anomalies/Matrix/SHIELD/Big_SHIELD_PRIOR_{}.npy".format(id),something)
...
def sample_kl(vae, decoder, vae_encoder, train_data, test_data, id, dataset_min, dataset_max, dataset_type): 
    ...
    truths = np.reshape(test_data, (len(test_data),170 * 79))
    something = losses(truths,sample_mean_var)
    np.save("/fast/gmooers/gmooers_git/CBRAIN-CAM/MAPS/DYAMOND_VAEs/model_graphs/Anomalies/Matrix/SHIELD/NORM_GEM_Interp_For_SHIELD_PRIOR_{}.npy".format(id),something)
    #np.save("/fast/gmooers/gmooers_git/CBRAIN-CAM/MAPS/DYAMOND_VAEs/model_graphs/Anomalies/Matrix/SPCAM/ARPEGE_Interp_For_SPCAM_ELBO_{}.npy".format(id),something)
    print("completed")     
