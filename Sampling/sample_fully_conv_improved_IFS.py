...
from train_fully_conv_IFS import encoder_gen, decoder_gen, kl_reconstruction_loss, ELBO_Test, ELBO_Test_Big, PRIOR_Test_Big, PRIOR_Test_Norm
import numpy as np
import gc 
...
def reconstruct_targets_paper(vae, test_data, targets, id, dataset_max, dataset_min):
    ...
    for target in targets:
        ...
        sample_mean = sample_mean_var[0, :128*137]
        sample_log_var = sample_mean_var[0, 128**137:]
        ...
        recon_mean = recon_mean.reshape((137, 128))
        recon_var = recon_var.reshape((137, 128))
...
def sample_latent_space_var(vae_encoder, train_data, test_data, id, dataset_min, dataset_max, dataset_type): 
    ...
    np.save("model_graphs/latent_space/Scree/Z_Test_IFS_Full_1024D_Latent_Space__{}".format(id), z_test)
    np.save("model_graphs/latent_space/Scree/Mean_Test_IFS_Full_1024D_Latent_Space__{}".format(id), test_mean)
    np.save("model_graphs/latent_space/Scree/Log_Var_Test_IFS_Full_1024D_Latent_Space__{}".format(id), test_log_var)
    
    np.save("model_graphs/latent_space/Scree/Z_Train_IFS_Full_1024D_Latent_Space__{}".format(id), z_train)
    np.save("model_graphs/latent_space/Scree/Mean_Train_IFS_Full_1024D_Latent_Space__{}".format(id), train_mean)
    np.save("model_graphs/latent_space/Scree/Log_Var_Train_IFS_Full_1024D_Latent_Space__{}".format(id), train_log_var)
    #print(dsfsdfsdf)
    # Apply scaling and tsne 
    sc = StandardScaler()
    ...
    np.save("model_graphs/latent_space/Scree/IFS_3D_PCA_Latent_Space__{}".format(id), z_test_pca)
...
def sample_frob_norm(vae, decoder, vae_encoder, train_data, test_data, id): 
    ...
    sample_mean = sample_mean_var[:, :128*137]
    truths = np.reshape(test_data, (len(test_data),137*128))
    ...
    np.save("Saved_Data/Small_IFS_MSE__{}.npy".format(id), np.mean(mses))
    np.save("Saved_Data/Small_IFS_SSIM__{}.npy".format(id), np.mean(ssim))
    print("completed")   
...
def sample_anon_detect(vae, decoder, vae_encoder, train_data, test_data, id, dataset_min, dataset_max, dataset_type): 
    ...
    sample_mean_var = decoder.predict(z_test) 
    sample_mean = sample_mean_var[:, :128*137]
    sample_log_var = sample_mean_var[:, 128*137:]
    ...
    #gc.collect()
    #print(elbo.shape)
    #np.save("model_graphs/Anomalies/NICAM_Interp_For_IFS_ELBO_{}.npy".format(id),something)
    np.save("model_graphs/Anomalies/Matrix/IFS/BIG_IFS_ELBO_{}.npy".format(id),something)
    print("completed")
...
def sample_prior_detect(vae, decoder, vae_encoder, train_data, test_data, id, dataset_min, dataset_max, dataset_type): 
    ...
    truths = np.reshape(test_data, (len(test_data),128*137))
    print("The reshaped test data is", truths.shape)
    something = losses(truths,sample_mean_var)
    print("New record")
    #sess = tf.InteractiveSession()
    ...
    #gc.collect()
    #print(elbo.shape)
    np.save("/fast/gmooers/gmooers_git/CBRAIN-CAM/MAPS/DYAMOND_VAEs/model_graphs/Anomalies/Matrix/IFS/BIG_IFS_PRIOR_{}.npy".format(id),something)
...
def sample_kl(vae, decoder, vae_encoder, train_data, test_data, id, dataset_min, dataset_max, dataset_type): 
    ...
    truths = np.reshape(test_data, (len(test_data),128*137))
    something = losses(truths,sample_mean_var)
    np.save("/fast/gmooers/gmooers_git/CBRAIN-CAM/MAPS/DYAMOND_VAEs/model_graphs/Anomalies/Matrix/IFS/NORM_GEM_Interp_For_IFS_PRIOR_{}.npy".format(id),something)
    #np.save("/fast/gmooers/gmooers_git/CBRAIN-CAM/MAPS/DYAMOND_VAEs/model_graphs/Anomalies/Matrix/SPCAM/ARPEGE_Interp_For_SPCAM_ELBO_{}.npy".format(id),something)
    print("completed")     
