...
from train_fully_conv_UM import encoder_gen, decoder_gen, kl_reconstruction_loss, ELBO_Test, ELBO_Test_Big, PRIOR_Test_Big, PRIOR_Test_Norm
import numpy as np
import gc 
...
def reconstruct_targets_paper(vae, test_data, targets, id, dataset_max, dataset_min):
    ...
    for target in targets:
        ...
        sample_mean = sample_mean_var[0, :102 * 55]
        sample_log_var = sample_mean_var[0, 102 * 55:]
        ...
        max_sample = np.max(sample.reshape((102 * 55,)))
        ...
        min_sample = np.min(sample.reshape((102 * 55,)))
        print("min of original", min_sample)

        # Reshape reconstructed sample 
        recon_mean = recon_mean.reshape((55, 102))
        recon_var = recon_var.reshape((55, 102))
...
def reconstruct_targets(vae, test_data, targets, id, dataset_max, dataset_min):
    ...
    for target in targets:
        ...
        sample = test_data[target]
        sample_mean_var = vae.predict(np.expand_dims(sample, 0))
        sample_mean = sample_mean_var[0, :128 * 58]
        sample_log_var = sample_mean_var[0, 128 * 58:]
...
def sample_reconstructions(vae, train_data, test_data, id, dataset_max, dataset_min): 
    ...
    for i in range(5):
        ...
        print("original sample", sample.reshape((102 * 55,)))
        print("reconstructed sample", recon_sample)
        print(np.max(np.abs(sample.reshape((102 * 55,)) - recon_sample)))
        ...
        min_sample = np.min(sample.reshape((102 * 55,)))
        print("min of original", min_sample)
        recon_sample = recon_sample.reshape((55, 102))
...
def sample_latent_space_var(vae_encoder, train_data, test_data, id, dataset_min, dataset_max, dataset_type): 
    ...
    #np.save("/fast/gmooers/gmooers_git/CBRAIN-CAM/MAPS/DYAMOND_VAEs/model_graphs/latent_space/UM/Big_SPCAM_Interp_For_UM_Full_1024D_Latent_Space__{}".format(id), z_test)
    #np.save("model_graphs/latent_space/Scree/Z_Test_UM_Full_1024D_Latent_Space__{}".format(id), z_test)
    #np.save("model_graphs/latent_space/Scree/Mean_Test_UM_Full_1024D_Latent_Space__{}".format(id), test_mean)
    #np.save("model_graphs/latent_space/Scree/Log_Var_Test_UM_Full_1024D_Latent_Space__{}".format(id), test_log_var)
    
    #np.save("model_graphs/latent_space/Scree/Z_Train_UM_Full_1024D_Latent_Space__{}".format(id), z_train)
    #np.save("model_graphs/latent_space/Scree/Mean_Train_UM_Full_1024D_Latent_Space__{}".format(id), train_mean)
    #np.save("model_graphs/latent_space/Scree/Log_Var_Train_UM_Full_1024D_Latent_Space__{}".format(id), train_log_var)
    sc = StandardScaler()
    ...
    #np.save("model_graphs/latent_space/Scree/UM_3D_PCA_Latent_Space__{}".format(id), z_test_pca)
    np.save("/fast/gmooers/gmooers_git/CBRAIN-CAM/MAPS/DYAMOND_VAEs/model_graphs/latent_space/UM/Big_UM_2D_PCA_Latent_Space__{}".format(id), z_test_pca)
...
def sample_frob_norm(vae, decoder, vae_encoder, train_data, test_data, id): 
    ...
    truths = np.reshape(test_data, (len(test_data),102 * 55))
    ...
    np.save("Saved_Data/Small_UM_MSE__{}.npy".format(id), np.mean(mses))
    np.save("Saved_Data/Small_UM_SSIM__{}.npy".format(id), np.mean(ssim))
    print("completed")   
...
def sample_anon_detect(vae, decoder, vae_encoder, train_data, test_data, id, dataset_min, dataset_max, dataset_type): 
    ...
    np.save("/fast/gmooers/gmooers_git/CBRAIN-CAM/MAPS/DYAMOND_VAEs/model_graphs/Anomalies/Matrix/UM/BIG_UM_ELBO_{}.npy".format(id),something)
    print("completed")
...
def sample_prior_detect(vae, decoder, vae_encoder, train_data, test_data, id, dataset_min, dataset_max, dataset_type): 
    ...
    #gc.collect()
    #print(elbo.shape)
    #np.save("/fast/gmooers/gmooers_git/CBRAIN-CAM/MAPS/DYAMOND_VAEs/model_graphs/Anomalies/Matrix/UM/BIG_UM_PRIOR_{}.npy".format(id),something)
...
def sample_kl(vae, decoder, vae_encoder, train_data, test_data, id, dataset_min, dataset_max, dataset_type): 
    ...
    truths = np.reshape(test_data, (len(test_data),102*55))
    something = losses(truths,sample_mean_var)
    np.save("/fast/gmooers/gmooers_git/CBRAIN-CAM/MAPS/DYAMOND_VAEs/model_graphs/Anomalies/Matrix/UM/NORM_GEM_Interp_For_UM_PRIOR_{}.npy".format(id),something)
    #np.save("/fast/gmooers/gmooers_git/CBRAIN-CAM/MAPS/DYAMOND_VAEs/model_graphs/Anomalies/Matrix/SPCAM/ARPEGE_Interp_For_SPCAM_ELBO_{}.npy".format(id),something)
    print("completed") 
