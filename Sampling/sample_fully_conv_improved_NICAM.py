...
from train_fully_conv_NICAM import encoder_gen, decoder_gen, kl_reconstruction_loss, ELBO_Test, ELBO_Test_Big, PRIOR_Test_Big, PRIOR_Test_Norm
import numpy as np
import gc 
...
def sample_latent_space_var(vae_encoder, train_data, test_data, id, dataset_min, dataset_max, dataset_type): 
    ...
    #np.save("model_graphs/latent_space/NICAM/Small_GEM_Interp_For_NICAM_Full_1024D_Latent_Space__{}".format(id), z_test)
    np.save("model_graphs/latent_space/Scree/Z_Test_NICAM_Full_1024D_Latent_Space__{}".format(id), z_test)
    np.save("model_graphs/latent_space/Scree/Mean_Test_NICAM_Full_1024D_Latent_Space__{}".format(id), test_mean)
    np.save("model_graphs/latent_space/Scree/Log_Var_Test_NICAM_Full_1024D_Latent_Space__{}".format(id), test_log_var)
    
    np.save("model_graphs/latent_space/Scree/Z_Train_NICAM_Full_1024D_Latent_Space__{}".format(id), z_train)
    np.save("model_graphs/latent_space/Scree/Mean_Train_NICAM_Full_1024D_Latent_Space__{}".format(id), train_mean)
    np.save("model_graphs/latent_space/Scree/Log_Var_Train_NICAM_Full_1024D_Latent_Space__{}".format(id), train_log_var)
    # Apply scaling and tsne 
    sc = StandardScaler()
    ...
    #np.save("model_graphs/latent_space/NICAM/Small_GEM_Interp_For_NICAM_3D_PCA_Latent_Space__{}".format(id), z_test_pca)
    np.save("model_graphs/latent_space/Scree/NICAM_3D_PCA_Latent_Space__{}".format(id), z_test_pca)
...
def sample_frob_norm(vae, decoder, vae_encoder, train_data, test_data, id): 
    ...
    sample_mean = sample_mean_var[:, :170*78]
    truths = np.reshape(test_data, (len(test_data),170*78))
    ...
    np.save("Saved_Data/Small_NICAM_SSIM__{}.npy".format(id), np.mean(ssim))
    print("completed")   
...
def sample_anon_detect(vae, decoder, vae_encoder, train_data, test_data, id, dataset_min, dataset_max, dataset_type): 
    ...
    sample_mean_var = decoder.predict(z_test) 
    sample_mean = sample_mean_var[:, :170*78]
    sample_log_var = sample_mean_var[:, 170*78:]
    ...
    np.save("model_graphs/Anomalies/Matrix/NICAM/BIG_NICAM_ELBO_{}.npy".format(id),something)
    print("completed")
...
def sample_prior_detect(vae, decoder, vae_encoder, train_data, test_data, id, dataset_min, dataset_max, dataset_type): 
    ...
    #gc.collect()
    #print(elbo.shape)
    np.save("/fast/gmooers/gmooers_git/CBRAIN-CAM/MAPS/DYAMOND_VAEs/model_graphs/Anomalies/Matrix/NICAM/BIG_NICAM_PRIOR_{}.npy".format(id),something)
...
def sample_kl(vae, decoder, vae_encoder, train_data, test_data, id, dataset_min, dataset_max, dataset_type): 
    ...
    truths = np.reshape(test_data, (len(test_data),170*78))
    something = losses(truths,sample_mean_var)
    np.save("/fast/gmooers/gmooers_git/CBRAIN-CAM/MAPS/DYAMOND_VAEs/model_graphs/Anomalies/Matrix/NICAM/NORM_GEM_Interp_For_NICAM_PRIOR_{}.npy".format(id),something)
    #np.save("/fast/gmooers/gmooers_git/CBRAIN-CAM/MAPS/DYAMOND_VAEs/model_graphs/Anomalies/Matrix/SPCAM/ARPEGE_Interp_For_SPCAM_ELBO_{}.npy".format(id),something)
    print("completed")     
